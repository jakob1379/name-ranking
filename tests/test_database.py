...
class TestDatabaseInitialization:
    ...
    def test_init_database_creates_tables(self, mock_db_path):
        ...
        # Arrange
        from st_name_ranking.database import init_database, get_connection
        ...
        with get_connection() as conn:
            ...
            expected_tables = {'names', 'ratings', 'region_mapping', 'source_versions', 'user_settings'}
    ...
    def test_region_mapping_populated(self, mock_db_path):
        ...
        with get_connection() as conn:
            ...
            cursor.execute("SELECT region FROM region_mapping WHERE nationality = 'Denmark'")
    ...
    def test_get_connection_context_manager(self, mock_db_path):
        ...
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("INSERT INTO names (name, gender) VALUES (?, ?)",
                          ("TestName", "Male"))
...
class TestNameOperations:
    ...
    def test_insert_names_via_sql(self, initialized_db):
        ...
        with get_connection() as conn:
            ...
            cursor.execute("SELECT gender FROM names WHERE name = ?", ("Peter",))
            result = cursor.fetchone()
...
class TestSyncOperations:
    ...
    def test_sync_names_with_submodule(self, mock_submodule_path, initialized_db):
        ...
        with patch('subprocess.run') as mock_run, \
             patch('st_name_ranking.data_loader.is_valid_name', side_effect=mock_is_valid_name):
            ...
            result = sync_names_with_submodule(submodule_path=mock_submodule_path)
